...
# create Components
curl -w "$FORMAT" http://$ADDRESS:10080/systemmanager/components/network0 -X PUT -d \
...
curl -w "$FORMAT" http://$ADDRESS:10080/systemmanager/components/aggregator -X PUT -d \
...
'{"id": "conn2", "type": "LogicAndNetwork", "connection_type": "original", "logic_id": "openflow_driver", "network_id":"network0"}'
curl -w "$FORMAT" http://$ADDRESS:10080/systemmanager/connections/conn3 -X PUT -d \
'{"id": "conn3", "type": "LogicAndNetwork", "connection_type": "aggregated", "logic_id": "aggregator", "network_id":"network1"}'
curl -w "$FORMAT" http://$ADDRESS:10080/systemmanager/connections/conn4 -X PUT -d \
'{"id": "conn4", "type": "LogicAndNetwork", "connection_type": "original", "logic_id": "aggregator", "network_id":"network0"}'
